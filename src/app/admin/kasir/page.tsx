    ...
    <>
      <div className="max-w-6xl mx-auto space-y-6 pb-12 print:hidden">
        <div>
          ...
          <p className="text-slate-500 mt-1">Sistem kasir toko fisik. Tembak barcode untuk mempercepat transaksi offline.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-md relative overflow-hidden">
              ...
              <label className="block text-sm font-medium text-slate-700 mb-2 flex items-center gap-2 relative z-10">
                <UserCircle className="w-5 h-5 text-brand-500" /> Petugas Kasir
              </label>
              <select 
                value={cashierName} 
                onChange={(e) => setCashierName(e.target.value)}
                className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all font-semibold relative z-10 shadow-sm"
              >
                <option value="" disabled>-- Pilih Nama Kasir --</option>
                {cashierList.map(name => <option key={name} value={name}>{name}</option>)}
              </select>
              
              <hr className="my-6 border-slate-100 relative z-10" />

              <label className="block text-sm font-medium text-slate-700 mb-2 relative z-10">Scan Barcode (SKU)</label>
              <form onSubmit={handleScan} className="relative z-10">
                <input 
                  ...
                  className={`w-full bg-white border-2 rounded-xl px-4 py-4 text-slate-900 text-xl font-mono tracking-widest focus:outline-none transition-all shadow-sm ${
                    !cashierName ? 'opacity-50 cursor-not-allowed border-slate-200' : 'border-slate-300 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10'
                  }`}
                  autoComplete="off"
                />
                <button type="submit" className="hidden">Scan</button>
              </form>
              <p className="text-xs text-slate-400 mt-3 relative z-10 leading-relaxed">Sistem otomatis mendeteksi saat scanner menembakkan kode dan akan langsung mencetak struk jika transaksi sukses.</p>
            </div>

            {/* Feedback Messages */}
            {error && (
              <div className="bg-red-50 rounded-xl p-4 border border-red-200 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 shadow-sm">
            ...
            {success && (
              <div className="bg-green-50 rounded-xl p-4 border border-green-200 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 shadow-sm">
          ...
          <div className="lg:col-span-2">
            {item ? (
              <div className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-md animate-in zoom-in-95 duration-300 flex flex-col h-full">
                <div className="p-6 bg-brand-50 border-b border-brand-100 flex justify-between items-center">
                  ...
                  <button 
                    ...
                    className="text-sm text-slate-500 hover:text-slate-900 flex items-center gap-1 transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-200 font-medium"
                  >
                ...
                <div className="p-8 flex flex-col md:flex-row gap-8 flex-grow">
                  <div className="w-full md:w-56 h-56 rounded-2xl overflow-hidden bg-slate-50 border border-slate-200 flex-shrink-0 relative">
                    {item.images[0] ? (
                  ...
                  <div className="flex-1 space-y-4">
                    <div>
                      <div className="text-sm font-mono text-brand-600 font-bold mb-1.5 tracking-widest bg-brand-50 inline-block px-2.5 py-1 rounded-md border border-brand-100">{item.sku}</div>
                      <h3 className="text-3xl font-black text-slate-900 mt-2 leading-tight">{item.title}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                      <div><span className="text-xs text-slate-500 block uppercase tracking-widest font-bold mb-1">Kategori</span><span className="font-semibold text-slate-900">{item.category}</span></div>
                      <div><span className="text-xs text-slate-500 block uppercase tracking-widest font-bold mb-1">Kondisi</span><span className="font-bold text-green-600">Grade {item.grade}</span></div>
                    </div>
                    
                    <div className="pt-4">
                      <span className="text-sm text-slate-500 block uppercase tracking-widest font-bold mb-1">Total Tagihan</span>
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-200 mt-auto">
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-5 rounded-xl bg-green-500 hover:bg-green-600 text-white font-black text-xl uppercase tracking-widest shadow-lg shadow-green-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
                  >
              ...
              <div className="h-full min-h-[500px] bg-white rounded-2xl border-2 border-slate-200 border-dashed shadow-md flex flex-col items-center justify-center text-center p-8">
                <div className="relative mb-6">
                  <div className="absolute inset-0 bg-slate-100 rounded-full blur-xl animate-pulse" />
                  <ScanBarcode className="w-24 h-24 text-slate-300 relative z-10" />
                </div>
                <h3 className="text-2xl font-semibold text-slate-800 mb-2 tracking-tight">Menunggu Pemindaian</h3>
