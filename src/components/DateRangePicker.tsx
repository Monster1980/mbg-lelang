...
export default function DateRangePicker({ value, onChange, placeholder = "Pilih rentang tanggal" }: Props) {
  ...
  return (
    <div className="relative inline-block" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 z-50 bg-white border border-slate-200 rounded-2xl shadow-xl p-0 flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 w-[480px]">
          {/* Quick Presets Panel */}
          <div className="w-full md:w-40 bg-slate-50/50 p-3 flex flex-col gap-1.5 justify-start">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 mb-1">Presets</p>
            <button
              type="button"
              id="preset-today"
              onClick={() => applyPreset("today")}
              className="text-left w-full px-2.5 py-1.5 rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors"
            >
              Hari Ini
            </button>
            <button
